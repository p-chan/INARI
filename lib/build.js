...
function parseFile (data, file) {
  ...
  post = {
    ...
    url : path.join('blog', path.basename(file, '.md') + '.html'),
...
function writeFile (data, file) {
  fs.writeFile(path.join(CWD + '/build/blog/' + path.basename(file, '.md') + '.html'), data, function (err) {
    count++;
    if(count >= max) {
...
function destAssets () {
  shell.cp('-r', path.join(VIEWS_DIR, 'assets'), BUILD_DIR);
