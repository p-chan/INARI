...
function getHTMLList () {
  fs.readdir(path.join(BUILD_DIR, 'blog'), function (err, files) {
...
function delPost (files) {
  ...
  files.forEach(function (file) {
    fs.unlink(path.join(BUILD_DIR, 'blog/') + file +'.html', function (err) {
      if (err) return console.log('ERR');
      console.log('[INFO]' + space + 'Deleted ' + file + '.html');
      count++;
      if (count >= max) {
        deployBlog();
      };
...
function push2ghp () {
  ...
  shell.exec('git commit -m "commit"');
  shell.exec('git checkout master');
  shell.exec('git push -u origin gh-pages');
  shell.exec('git checkout master');
}